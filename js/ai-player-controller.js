...
class AIPlayerController {    
    ...
    initialize() {
        ...
        window.addEventListener('aiDifficultyChanged', (event) => {
            ...
            this.turnInterval = gameConfig.getAITurnInterval();
            console.log(`AI difficulty changed to: ${this.aiDifficulty}, turn interval: ${this.turnInterval}ms`);
    ...
    initializeTargetStateGroups() {
        ...
        console.log(`AI (${this.aiDifficulty}) targeting state groups:`, this.targetStateGroups);
    }    startAITurnLoop() {
        console.log(`Starting AI turn loop with interval: ${this.turnInterval}ms, difficulty: ${this.aiDifficulty}, active: ${this.aiActive}`);
        ...
        this.turnTimer = setInterval(() => {
            ...
            if (this.aiActive && gameOptions.gameplay) {
                console.log(`Taking AI turn (${this.aiDifficulty}) with interval: ${this.turnInterval}ms`);
                this.takeAITurn();
            } else {
                console.log(`Skipping AI turn - active: ${this.aiActive}, gameplay: ${gameOptions.gameplay}`);
            }
        }, this.turnInterval);
        ...
        try {
            ...
            if (!this.statesData) {
                try {
                    ...
                    if (!response.ok) {
                        throw new Error(`Failed to fetch states data: ${response.status} ${response.statusText}`);
                    }
    ...
    async takeMediumDifficultyTurn() {
        ...
        if (leadingCampaign && Math.random() < 0.6) {
            ...
            if (success) {
                console.log(`AI (MEDIUM) contributed to leading campaign: ${category}-${index + 1}`);
                return;
            }
        }
        ...
        try {
            ...
            // First check if there are any campaigns where AI is leading and nearly complete
            const leadingCampaign = this.findLeadingCampaign();
            console.log('Leading campaign check:', leadingCampaign ? `Found in ${leadingCampaign.category}` : 'None found');
            
            if (leadingCampaign && Math.random() < 0.7) {
                // 70% chance to prioritize leading campaign (higher than MEDIUM)
                const { category, index } = leadingCampaign;
                console.log(`Attempting to contribute to leading campaign: ${category}-${index + 1}`);
                const success = window.incrementPolicy(category, index, this.aiPlayerId);
                
                if (success) {
                    console.log(`AI (HARD) contributed to leading campaign: ${category}-${index + 1}`);
                    return;
                } else {
            ...
            console.log('Close to completion campaign check:', closeToCompletionCampaign ? `Found in ${closeToCompletionCampaign.category}` : 'None found');
            
            if (closeToCompletionCampaign && Math.random() < 0.4) {
                const { category, index } = closeToCompletionCampaign;
                console.log(`Attempting to contribute to campaign close to completion: ${category}-${index + 1}`);
                const success = window.incrementPolicy(category, index, this.aiPlayerId);
                
                if (success) {
                    console.log(`AI (HARD) contributed to campaign close to completion: ${category}-${index + 1}`);
                    return;
                } else {
            ...
            if (randomAction < 0.35) {
                ...
                if (!rallyPlaced) {
                    // If rally placement failed, fall back to state targeting
                    console.log('Rally placement failed, falling back to state targeting');
    ...
    async placeStrategyRally() {
        console.log(`AI (${this.aiDifficulty}) attempting to place strategic rally`);
        ...
        if (targetState) {
            return await rallyController.handleRallyPlacement(targetState, this.aiPlayerId);
        }
    ...
    findSuitableStatesForStrategicRally() {
        ...
        for (const stateData of this.statesData || []) {
            ...
            if (this.aiDifficulty === 'HARD') {
                ...
                if (player1Popularity > player2Popularity && player1Popularity - player2Popularity < 15) {
                    strategicValue += 30;
                }
            }
    ...
    chooseTargetStateInFocusGroups(moreStrategic = false) {
        ...
        return targetGroupStates[Math.floor(Math.random() * targetGroupStates.length)];
    }
    ...
    getGroupsForState(stateData) {
        ...
        if (stateData.AgriculturalRegion === "TRUE") groups.push('Agricultural Region');
        if (stateData.BorderLands === "TRUE") groups.push('Border Lands');
        if (stateData.Pilgrimage === "TRUE") groups.push('Pilgrimage');
        if (stateData.IndustrialCorridor === "TRUE") groups.push('Industrial Corridor');
        ...
        if (stateData.TravelAndTourism === "TRUE") groups.push('Travel and Tourism');
