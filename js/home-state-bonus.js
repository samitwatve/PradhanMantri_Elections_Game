...
class HomeStateBonus {
    ...
    async initialize() {
        ...
        try {
            ...
            if (storedConfig) {
                ...
                console.log(`Initialized home states - Player 1: ${this.player1HomeState}, Player 2: ${this.player2HomeState}`);
     ...
     */    isHomeState(playerId, stateName) {
        ...
        const playerHomeState = playerId === 1 ? this.player1HomeState : this.player2HomeState;
        ...
        console.log(`Checking home state for Player ${playerId}: "${stateName}" against "${playerHomeState}". Result: ${isHome}`);
     ...
     */    getCampaignCost(playerId, stateName, baseCost) {
        console.log(`Checking campaign cost for Player ${playerId} in ${stateName}. Base cost: ${baseCost}M`);
        ...
        if (this.isHomeState(playerId, stateName)) {
            ...
            console.log(`%cHome state discount applied for Player ${playerId} in ${stateName}. Original cost: ${baseCost}M, Discounted: ${discountedCost}M`, 'color: gold; font-weight: bold');
            return discountedCost;
        }
        
        console.log(`No home state discount for Player ${playerId} in ${stateName}. Cost remains: ${baseCost}M`);
        return baseCost;
    }
     ...
     */    applyHomeStateBonus(stateId, stateName, popularity) {
        console.log(`Applying home state bonus check for ${stateName} (${stateId})`);
        ...
        if (this.isHomeState(1, stateName)) {
            console.log(`%cApplying Player 1 home state bonus for ${stateName}`, 'color: orange; font-weight: bold');
            ...
            console.log(`Current P1: ${currentP1}%, Target: ${targetP1}%, Bonus: ${bonusAmount}%`);
            ...
            console.log(`Applied home state bonus for Player 1 in ${stateName}. New popularity: ${updatedPopularity.player1}%`);
        }
          // Apply home state bonus for Player 2
        if (this.isHomeState(2, stateName)) {
            console.log(`%cApplying Player 2 home state bonus for ${stateName}`, 'color: green; font-weight: bold');
            ...
            console.log(`Current P2: ${currentP2}%, Target: ${targetP2}%, Bonus: ${bonusAmount}%`);
            ...
            console.log(`Applied home state bonus for Player 2 in ${stateName}. New popularity: ${updatedPopularity.player2}%`);
        }
        ...
        const total = updatedPopularity.player1 + updatedPopularity.player2 + updatedPopularity.others;
        if (total !== 100) {
    ...
    debugCheckAllStates(statesData) {
        ...
        statesData.forEach(state => {
            ...
            if (isP1HomeState || isP2HomeState) {
                ...
                console.log(`  - Base cost: ${baseCost}M, P1 cost: ${p1Cost}M, P2 cost: ${p2Cost}M`);
            }
        });
    }
}
