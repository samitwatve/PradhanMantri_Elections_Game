...
class GameConfig {
    ...
    init() {
        ...
        this.settings = {
            ...
            player1StartingFunds: 250,
            player2StartingFunds: 250,
            
            // Player funds refresh per phase
            player1RefreshFunds: 250,
            player2RefreshFunds: 250,
    ...
    applyDebugAI() {
        ...
        if (window.gameTimer && this.debugMode) {
            console.log(`Debug mode: Updating timer from ${window.gameTimer.totalPhases} to ${this.settings.debug.totalPhases} phases`);
            ...
            window.gameTimer.totalDuration = window.gameTimer.totalPhases * window.gameTimer.phaseDuration;
            
            // If we're in the middle of the game, adjust remaining time proportionally
            const currentPhaseProgress = 1 - (window.gameTimer.phaseTimeRemaining / window.gameTimer.phaseDuration);
            const newPhaseNumber = Math.min(window.gameTimer.currentPhase, this.settings.debug.totalPhases);
            
            // Recalculate remaining time based on new total phases
            const remainingPhases = this.settings.debug.totalPhases - newPhaseNumber;
            const remainingTimeInCurrentPhase = window.gameTimer.phaseDuration * (1 - currentPhaseProgress);
            window.gameTimer.remainingTime = (remainingPhases * window.gameTimer.phaseDuration) + remainingTimeInCurrentPhase;
            
            // Update display
            window.gameTimer.updateDisplay();
            
            console.log(`Timer debug update complete:`);
            console.log(`- Old total phases: ${oldTotalPhases}, New total phases: ${window.gameTimer.totalPhases}`);
            console.log(`- Current phase: ${window.gameTimer.currentPhase}/${window.gameTimer.totalPhases}`);
        ...
        if (this.debugMode && this.settings.debug.oneClickMaxPopularity) {
            console.log('Debug mode: One-click max popularity enabled for map interactions');
        } else {
    ...
    disableOneClickMaxPopularity() {
        ...
        if (window.gameTimer) {
            console.log(`Restoring normal timer: ${this.settings.totalPhases} phases`);
            ...
            const currentPhaseProgress = 1 - (window.gameTimer.phaseTimeRemaining / window.gameTimer.phaseDuration);
            const remainingPhases = this.settings.totalPhases - window.gameTimer.currentPhase;
            const remainingTimeInCurrentPhase = window.gameTimer.phaseDuration * (1 - currentPhaseProgress);
            window.gameTimer.remainingTime = (remainingPhases * window.gameTimer.phaseDuration) + remainingTimeInCurrentPhase;
    ...
    getTotalPhases() {
        return this.debugMode ? this.settings.debug.totalPhases : this.settings.totalPhases;
    }
    ...
    getPlayer1StartingFunds() {
        return this.debugMode ? this.settings.debug.player1StartingFunds : this.settings.player1StartingFunds;
    }

    getPlayer2StartingFunds() {
        return this.debugMode ? this.settings.debug.player2StartingFunds : this.settings.player2StartingFunds;
    }

    getPlayer1RefreshFunds() {
        return this.debugMode ? this.settings.debug.player1RefreshFunds : this.settings.player1RefreshFunds;
    }

    getPlayer2RefreshFunds() {
        return this.debugMode ? this.settings.debug.player2RefreshFunds : this.settings.player2RefreshFunds;
    }

    isAIEnabled() {
        return this.debugMode ? this.settings.debug.aiEnabled : this.settings.aiEnabled;
    }
